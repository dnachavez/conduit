...
export function SystemLogsCard({
  ...
  const formatTimestamp = (timestamp: string) => {
    ...
    return (
      <div className="text-xs text-zinc-50 whitespace-nowrap">
        {dateStr} {timeStr}
      </div>
    );
  }
  ...
  return (
    <div className={cn("rounded-xl bg-[#171717] p-6", className)} {...props}>
      <div className="mb-6">
        <h3 className="text-base font-semibold mb-1 text-zinc-50">System Logs</h3>
        ...
        <table className="w-full table-fixed">
          ...
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Log ID</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Timestamp</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">User</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Role</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Event</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Module</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Description</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">Status</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400">IP</th>
              <th className="px-2 py-2 text-left text-xs font-medium text-zinc-400"></th>
            </tr>
          </thead>
          <tbody>
            {systemLogs.map((log) => (
              ...
              >
                ...
                <td className="px-2 py-2">{formatTimestamp(log.timestamp)}</td>
                <td className="px-2 py-2">
                  <div className="text-xs font-medium text-zinc-50 truncate">{log.user}</div>
                </td>
                <td className="px-2 py-2 text-xs">{renderUserRole(log.role)}</td>
                <td className="px-2 py-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-1">
                        {getEventTypeIcon(log.eventType)}
                        <span className="text-xs capitalize text-zinc-50 truncate">{log.eventType.replace('_', ' ')}</span>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="text-xs">{log.eventType.replace('_', ' ')}</p>
                    </TooltipContent>
                  </Tooltip>
                </td>
                <td className="px-2 py-2">
                  <div className="text-xs text-zinc-50 truncate">{log.module}</div>
                </td>
                <td className="px-2 py-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="text-xs max-w-full truncate cursor-help text-zinc-50">
                        {log.description}
                      </div>
                    </TooltipTrigger>
                    <TooltipContent side="top" className="max-w-[300px] p-3 bg-[#1f1f1f] border-zinc-800">
                      <p className="text-xs text-zinc-50">{log.description}</p>
                    </TooltipContent>
                  </Tooltip>
                </td>
                <td className="px-2 py-2">{getStatusBadge(log.status)}</td>
                <td className="px-2 py-2">
                  <div className="text-xs font-mono text-zinc-50 truncate">{log.ipAddress}</div>
                </td>
                <td className="px-2 py-2">
                  <div className="flex items-center justify-center">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-[200px] bg-[#1f1f1f] border-zinc-800">
                        <DropdownMenuLabel className="text-xs font-semibold text-zinc-50">Log Actions</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={(e) => handleViewDetails(log.id, e)} className="text-xs text-zinc-50">
                          <Eye className="mr-2 size-3.5" />
                          <span>View Details</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => handleDownloadLog(log.id, e)} className="text-xs text-zinc-50">
                          <Download className="mr-2 size-3.5" />
                          <span>Download Log</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={(e) => handleAddNote(log.id, e)} className="text-xs text-zinc-50">
